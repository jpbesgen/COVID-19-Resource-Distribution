describe("Basic features", function () {
          ...
          success(
            usersArray.filter(function (user) {
              var containsSearchTerm =
                user.fullname.toLowerCase().indexOf(term.toLowerCase()) != -1;
  ...
  it("Should have rendered the comments", function () {
    ...
    expect(
      $("#comment-list li.comment[data-id=8] .name .reply-to").text()
    ).toBe("Jack Hemsworth");
    expect(
      $("#comment-list li.comment[data-id=9] .name .reply-to").text()
    ).toBe("You");
    expect(
      $("#comment-list li.comment[data-id=5] .name .reply-to").text()
    ).toBe("Todd Brown");
    expect(
      $("#comment-list li.comment[data-id=10] .name .reply-to").text()
    ).toBe("Bryan Connery");
    ...
    var editedDateFromUI = new Date(
      $("#comment-list li.comment[data-id=9] .content .edited").attr(
        "data-original"
      )
    );
  ...
  it("Should have appended the child comments under their outermost parent", function () {
    ...
    checkOrder(
      $("#comment-list li.comment[data-id=1] .child-comments > li.comment"),
      [6, 7, 8, 9, 10]
    );
    checkOrder(
      $("#comment-list li.comment[data-id=2] .child-comments > li.comment"),
      []
    );
    checkOrder(
      $("#comment-list li.comment[data-id=3] .child-comments > li.comment"),
      [4, 5]
    );
  });
  ...
  it("Should be able to toggle all replies", function () {
    var toggleAll = $("#comment-list li.comment[data-id=1]").find(
      ".child-comments li.toggle-all"
    );
    expect(toggleAll.length).toBe(1);
    expect(toggleAll.text()).toBe("View all 5 replies");
    expect(
      $("#comment-list li.comment[data-id=1] li.comment:visible").length
    ).toBe(2);
  ...
  describe("Replying", function () {
    ...
    it("Should be able to reply", function () {
      ...
      var lastChild = mostPopularComment
        .find(".child-comments")
        .children()
        .last();
      ...
      run(function () {
        ...
        // Check position
        checkOrder(mostPopularComment.find("li.comment"), [
          6,
          7,
          8,
          9,
    ...
    it("Should be able to re-reply", function () {
      var childComment = mostPopularComment.find(
        ".child-comments li.comment[data-id=9]"
      );
      ...
      run(function () {
        ...
        expect(
          commentEl.find(".name .reply-to").text().indexOf("Bryan Connery")
        ).not.toBe(-1);
        expect(commentEl.find(".content").text()).toBe(replyText);
        expect(commentEl.hasClass("by-current-user")).toBe(true);
        checkCommentElementData(commentEl);
    ...
    it("Should be able to re-reply to a hidden reply", function () {
      ...
      var childComment = mostPopularComment
        .find(".child-comments li.comment")
        .first();
      childComment.find(".reply").first().click();
      ...
      run(function () {
        ...
        expect(
          commentEl.find(".name .reply-to").text().indexOf("Jack Hemsworth")
        ).not.toBe(-1);
        expect(commentEl.find(".content").text()).toBe(replyText);
        expect(commentEl.hasClass("by-current-user")).toBe(true);
        checkCommentElementData(commentEl);
    ...
    it("Should reply to original user when erasing the reply-to tag", function () {
      ...
      expect(parseInt(textarea.attr("data-parent"))).toBe(
        childComment.data().model.id
      );
  ...
  describe("Editing", function () {
    ...
    it("Should be able to open and close the edit field", function () {
      ...
      ownComment
        .find("> .comment-wrapper > *:not(.commenting-field)")
  ...
  describe("Deleting", function () {
    ...
    it("Should be able to delete a main level comment", function () {
      var commentId = 3;
      var ownComment = $(
        '#comment-list li.comment[data-id="' + commentId + '"]'
      );
      ...
      run(function () {
        expect(comments.getComments().length).toBe(
          commentCountBeforeDelete - 3
        );
        ...
        $(childComments).each(function (index, id) {
          ...
          expect(
            $('#comment-list li.comment[data-id="' + id + '"]').length
          ).toBe(0);
        });
        ...
        expect(
          $('#comment-list li.comment[data-id="' + commentId + '"]').length
        ).toBe(0);
      });
    });
    ...
    it("Should be able to delete a reply", function () {
      ...
      expect(
        comments.commentsById[outermostParent.attr("data-id")].childs.length
      ).toBe(5);
  ...
  describe("Uploading attachments", function () {
    ...
    it("Should able to upload a new main level attachment", function () {
      ...
      wait(function () {
        var commentCountUpdated =
          comments.getComments().length == commentCount + 1;
        var attachmentCountUpdated =
          comments.getAttachments().length == attachmentCount + 1;
      ...
      run(function () {
        ...
        expect($('li.comment[data-id="' + idOfNewComment + '"]').length).toBe(
          2
        );
  ...
  function checkCommentElementData(commentEl) {
    ...
    // Fields to be tested
    var profilePictureURL = commentEl
  ...
  function getOrder(elements) {
    return elements
      .map(function (index, commentEl) {
        return $(commentEl).data().id;
      })
      .toArray();
  }
    ...
    run(function () {
      ...
      $(Object.keys(ownCommentModel)).each(function (index, key) {
        if (key == "content" || key == "modified") {
          ...
          expect(JSON.stringify(ownCommentModel[key])).toBe(
            JSON.stringify(ownCommentModelBefore[key])
          );
        } else {
