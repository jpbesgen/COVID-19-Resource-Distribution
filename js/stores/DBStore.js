class DatabaseStore {
    ...
    for (const commentID of designCommentIDs) {
      let result = await new Promise((res, rej) => {
        db.collection("Comments")
          ...
          .get()
          .then((snapshot) => {
            res(snapshot.exists ? snapshot.data() : {});
          });
      });
  ...
  async addComment(design_id, comment_value) {
    ...
    if (!this.isAuthenticated())
      throw new Error("Please login before making a comment!");
  ...
  async removeComment(comment_id) {
    // Check if the user is authenticated, if not throw an error
    if (!this.isAuthenticated())
      throw new Error("Please login to remove your comments!");
      ...
      .then((snapshot) => {
        ...
        db.collection("Designs")
          ...
          .then((snapshot) => {
            let doc = snapshot.data();
            doc.upvotes =
              doc.upvotes - increment > 0 ? doc.upvotes - increment : 0;
            db.collection("Designs").doc(design_id).set(doc);
          });
      });
  }
  ...
  async removeCommentUpvote(user_id, comment_id) {
    try {
      ...
      upvotedList.splice(indexOfComment, 1);
      await db.collection("Users").doc(user_id).update({
        upvotedComments: upvotedList,
      });
  ...
  async getProfileUrl(uid) {
    ...
    try {
      ...
      return userRef.exists && userRef.data().hasOwnProperty("photoUrl")
