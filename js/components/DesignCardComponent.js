class DesignCardComponent extends Component {
  constructor(id, props) {
    ...
    EventStore.on(
      "DesignCardChange-" + this.props.design.id,
      this.handleDesignCardUpdated
    );
    ...
    let cardfrontUpvotesDisplay = new CardfrontUpvotesComponent(
      this.cardfront_upvotes_id,
      {
        upvotes,
      }
    );
  ...
  render() {
    ...
    if (design.attachments != null && design.attachments.length > 0) {
      ...
      design.attachments.forEach((attachment) => {
        downloads += `<a href="${attachment.url}" target="_blank" rel="noopener noreferrer" class="card-attachment" design-id="${design.id}" download> ${attachment.name}</a>`;
    ...
    if (design.links != null && design.links.length > 0) {
      ...
      design.links.forEach((link) => {
        links += `<a href="${link}" class="card-link" design-id="${design.id}" target="_blank" rel="noopener noreferrer"> ${link}</a>`;
    ...
    return `
            ...
            <div class="card-body">
                ...
                ${printerRequired}
                ${certification}
                <button class="btn btn-block card-text" data-toggle="modal" data-target="#${
                  design.id
                }">See More</button>
            </div>
            <div class="btn-group">
                ...
                <span class="btn" id="${this.cardfront_upvotes_id}">${
      design.upvotes
    }</span>
      ...
      design.upvotes
                            ...
                            <ul class="nav nav-tabs nav-justified navbar-dark bg-dark" id="myTab" role="tablist">
                                <li class="nav-item" id="details-tab-${
                                  design.id
                                }">
                                    <a class="nav-link active show" id="details-tab-link" data-toggle="tab" href="#details-page-${
                                      design.id
                                    }" role="tab" aria-controls="details-page-${
      design.id
    }" aria-selected="true">Details</a>
                                </li>
                                <li class="nav-item" id="comments-tab-${
                                  design.id
                                }">
                                    <a class="nav-link show" id="comments-tab-link" data-toggle="tab" href="#comments-page-${
                                      design.id
                                    }" role="tab" aria-controls="comments-page-${
      design.id
                            ...
                            <div class="tab-content" id="tab-content-${
                              design.id
                                ...
                                <div class="tab-pane fade show active" id="details-page-${
                                  design.id
                                }" role="tabpanel" aria-labelledby="details">
                                    <p class="modal-text"><b>Category</b><br />${categoryDisplayName}</p>
                                    <p class="modal-text"><b>Description</b><br />${
                                      design.description
                                    }</p>
                                    ${
                                      design.printerRequired
                                        ? '<p class="modal-text"><b>3D Printer Required</b><br /></p>'
                                        : ""
                                    }
                                    <p class="modal-text"><b>Certified</b><br /> ${
                                      design.certified
                                    }</p>
                                    ${
                                      design.certifiedLink != null &&
                                    ...
                                    <p class="modal-text"><b>Difficulty Level</b><br /> ${
                                      design.difficulty
                                    }</p>
                                    ${
                                      design.credit
                                ...
                                <div class="tab-pane fade" id="comments-page-${
                                  design.id
