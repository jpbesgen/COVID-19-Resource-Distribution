...
const CustomItem = React.forwardRef(({ children, onClick, isActive }, ref) => (
	...
	>
		<svg
			...
			viewBox="0 0 12 12"
			fill="none"
			className="makerspace-filter-item-checkbox"
		>
			{isActive ? (
				<path
					d="M3 5.53333L4.87805 8L10 4"
					stroke="#6BA48C"
					strokeOpacity="0.6"
					strokeWidth="1.5"
				/>
...
function TagDropdown(props) {
	...
	return (
		<Dropdown className="makerspace-filter">
			<Dropdown.Toggle as={CustomToggle}>{props.name}</Dropdown.Toggle>
...
export default class MakerspaceFilter extends Component {
	constructor(props) {
		...
		this.tagsCategories.set("Difficulty", ["Easy", "Medium", "Difficult"]);
		this.tagsCategories.set("Materials", ["Fabric", "Elastic"]);
	...
	render() {
		...
		return (
			<div className="filters-block">
				<div className="filters-dropdowns-block">{dropdowns}</div>
